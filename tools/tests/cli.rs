...
#[test]
fn visitor_emits_trait_to_stdout() {
    let path = write_tmp("ts_bnf_visitor_stdout.bnf", CLEAN_BNF);
    let out = tool().args(["visitor"]).arg(&path).output().unwrap();
    assert!(out.status.success());
    let stdout = String::from_utf8(out.stdout).unwrap();
    assert!(
        stdout.contains("pub trait Visitor<'tree>"),
        "stdout must contain the generated trait"
    );
    assert!(
        stdout.contains("fn visit_expr("),
        "stdout must contain a per-kind method for the input grammar"
    );
}

#[test]
fn visitor_output_file_flag() {
    let path = write_tmp("ts_bnf_visitor_out.bnf", CLEAN_BNF);
    let out_path = std::env::temp_dir().join("ts_bnf_visitor_out.rs");
    let _ = std::fs::remove_file(&out_path);
    let out = tool()
        .args(["visitor", "-o"])
        .arg(&out_path)
    ...
    assert!(out_path.exists(), "-o must create the output file");
    let content = std::fs::read_to_string(&out_path).unwrap();
    assert!(content.contains("pub trait Visitor<'tree>"));
    let _ = std::fs::remove_file(&out_path);
}

#[test]
fn visitor_name_defaults_to_filename_stem() {
    let path = write_tmp("ts_bnf_visitor_stem_default.bnf", CLEAN_BNF);
    let out = tool().args(["visitor"]).arg(&path).output().unwrap();
    assert!(out.status.success());
    let stdout = String::from_utf8(out.stdout).unwrap();
    assert!(
        stdout.contains(
            "ANTLR-style visitor for the `ts_bnf_visitor_stem_default` tree-sitter grammar."
        ),
        "grammar name must default to the filename stem"
    );
}

#[test]
fn visitor_name_flag_overrides_filename_stem() {
    let path = write_tmp("ts_bnf_visitor_stem_override.bnf", CLEAN_BNF);
    let out = tool()
        .args(["visitor", "--name", "mygrammar"])
        .arg(&path)
        .output()
        .unwrap();
    assert!(out.status.success());
    let stdout = String::from_utf8(out.stdout).unwrap();
    assert!(
        stdout.contains("ANTLR-style visitor for the `mygrammar` tree-sitter grammar."),
        "--name must override the filename stem"
    );
    // The filename stem legitimately still appears in the header's "from
    // <path>" line regardless of --name — only the trait's own doc text
    // (checked above) is affected by the override.
    assert!(
        !stdout.contains("for the `ts_bnf_visitor_stem_override`"),
        "the filename stem must not leak into the trait's doc text when --name is given"
    );
}

#[test]
fn visitor_header_present_by_default() {
    let path = write_tmp("ts_bnf_visitor_header_on.bnf", CLEAN_BNF);
    let out = tool().args(["visitor"]).arg(&path).output().unwrap();
    assert!(out.status.success());
    let stdout = String::from_utf8(out.stdout).unwrap();
    assert!(
        stdout.starts_with("// Generated by ts-bnf-tool"),
        "header comment must be present by default"
    );
}

#[test]
fn visitor_no_header_suppresses_comment() {
    let path = write_tmp("ts_bnf_visitor_header_off.bnf", CLEAN_BNF);
    let out = tool()
        .args(["visitor", "--no-header"])
        .arg(&path)
        .output()
        .unwrap();
    assert!(out.status.success());
    let stdout = String::from_utf8(out.stdout).unwrap();
    assert!(
        !stdout.contains("Generated by ts-bnf-tool"),
        "--no-header must suppress the generated-file comment"
    );
    assert!(
        stdout.starts_with("use tree_sitter::Node;"),
        "output must start directly with the use statement"
    );
}
...
/// both offending kinds, same as `visitor`'s own collision path, without
/// ever touching the filesystem or spawning `tree-sitter`.
...
/// this subcommand hand-authors, same as `visitor`'s own `--no-header`.
...
/// `tests/visitor.rs` uses for the plain `visitor` subcommand).
