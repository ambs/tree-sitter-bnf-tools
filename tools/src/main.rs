...
fn run_library(
    ...
    fs::write(dir.join("Cargo.toml"), library_cargo_toml(name))?;
    ...
    fs::write(examples_dir.join("walk.rs"), library_walk_example(name))?;
...
/// Renders `examples/walk.rs`: a `Counter` visitor that counts every node by
/// overriding only [`Visitor::combine`] — the one method the trait doesn't
/// give a default for — proving the crate works without writing any
/// per-kind override.
fn library_walk_example(name: &str) -> String {
    let crate_name = name.replace('-', "_");
    formatdoc! {r#"
        //! Parses a file with the `{name}` parser and prints how many nodes it
        //! contains, using nothing but the generated `Visitor` trait's required
        //! `combine` method. Run it with:
        //!
        //! ```sh
