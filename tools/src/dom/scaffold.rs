...
mod tests {
    ...
    fn s<'a>(grammar: &'a Grammar, name: &'a str) -> Scaffold<'a> {
        Scaffold { grammar, name, source: "test.bnf", no_header: true }
    }
    ...
    fn scaffold_header_present_by_default() {
        let g = Grammar::from_rules([p("a", TerminalLiteral("'x'".into()))]);
        let out = Scaffold { grammar: &g, name: "g", source: "grammar.bnf", no_header: false }
            .to_string();
        let version = env!("CARGO_PKG_VERSION");
    ...
    fn scaffold_no_header_suppresses_comment() {
        let g = Grammar::from_rules([p("a", TerminalLiteral("'x'".into()))]);
        let out = Scaffold { grammar: &g, name: "g", source: "grammar.bnf", no_header: true }
            .to_string();
    ...
    fn scaffold_header_uses_stdin_source() {
        let g = Grammar::from_rules([p("a", TerminalLiteral("'x'".into()))]);
        let out = Scaffold { grammar: &g, name: "g", source: "<stdin>", no_header: false }
            .to_string();
