...
impl GrammarJs<'_> {
    ...
    ///
    /// Also emits the `/// <reference types="tree-sitter-cli/dsl" />` + `// @ts-check`
    /// preamble that `tree-sitter init` itself puts at the top of a fresh
    /// `grammar.js` — it gives editor IntelliSense and `tsc`-based type checking of
    /// the DSL calls (`$`, `seq`, `choice`, `prec`, …) to anyone who opens or
    /// hand-tweaks the generated file (#322). Tied to the same `no_header` flag as
    /// the generated-by comment: both are "the header", suppressed together.
    fn fmt_header(&self, f: &mut Formatter<'_>) -> fmt::Result {
        if !self.no_header {
            ...
            writeln!(f, "/// <reference types=\"tree-sitter-cli/dsl\" />")?;
            writeln!(f, "// @ts-check")?;
        }
        Ok(())
    }
...
mod tests {
    ...
    fn scaffold_no_header_suppresses_comment() {
        ...
        assert!(!out.contains("tree-sitter-cli/dsl"));
        assert!(!out.contains("@ts-check"));
        assert!(out.starts_with("module.exports = grammar({"));
    }
