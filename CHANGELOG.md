...
- `convert`'s generated `grammar.js` now includes the standard
  ...
  and `// @ts-check` — right after the existing "Generated by ts-bnf-tool …"
  header comment, giving editor IntelliSense and `tsc`-based type checking
  of the DSL calls (`$`, `seq`, `choice`, `prec`, …) to anyone who opens or
  hand-tweaks the generated file. Suppressed by `--no-header` along with the
  rest of the header (#322).
