...
#### `ts-bnf-tool`
- `check`: warns about rules that are never referenced by any other rule or
  directive (unreachable rule detection, issue #40). The root rule (first in
  the file) is always exempt. Rules listed in `%extras`, `%inline`,
  `%supertypes`, or `%conflicts` directives are not flagged.
