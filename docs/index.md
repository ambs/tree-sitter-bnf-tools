...
11. [Generating a processing library](tutorial/11-generating-a-library.md) — `library`
...
and its diagrams are generated by the tool itself. `make bnf-self-check`
verifies that converting this file reproduces the same grammar as the
hand-maintained `tree-sitter-bnf/grammar.js`, so the two can't silently drift:
